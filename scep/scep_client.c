 ...
 * @section Description
 *
 * SCEP is a certificate enrolment protocol that leverages existing technology
...
#if (SCEP_CLIENT_TLS_SUPPORT == ENABLED)
         //Since SCEP messages are already cryptographically secured, it does
         //not require TLS (refer to RFC 8894, section 7.10)
         if(context->tlsInitCallback != NULL)
         {
            //Register TLS initialization callback
            error = httpClientRegisterTlsInitCallback(&context->httpClientContext,
               scepClientInitTlsContext, context);
         }
#endif
         //Check status code
         if(!error)
